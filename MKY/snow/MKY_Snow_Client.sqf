...
MKY_fnc_set_Wind_Velocity = {
	...
	if (isNil "varSnowData") then {
		...
		// arCurrent_Wind = [((random(1) + _intV) * ((varSnowData select 1) select 0)),((random(1) + _intV) * ((varSnowData select 1) select 1)),true];
	...
	diag_log format ["dir of bucket/player = %1 // %2",(getDir objEmitterHost),(getDir player)];
	diag_log format ["---->> current wind ---->> %1 ---->>",arCurrent_Wind];
	diag_log format ["---->> wind data ---->> %1 ---->>",varSnowData];
	(true);
};
...
MKY_fnc_light_Init = {
	...
	// light_parent setDropInterval 0.003;
	...
	// light_front setDropInterval 0.004;
	...
	// light_rear setDropInterval 0.009;
	...
	// light_right setDropInterval 0.004;
	...
	// light_left setDropInterval 0.009;
...
MKY_fnc_blizzard_Init = {
	...
	blizzard_parent setParticleParams [["a3\data_f\ParticleEffects\Universal\Universal.p3d", 16,13,6,0],"","Billboard",1,10,[0,30,8],[0,0,0],(0),1.69,1,1.5,[3],[[0,0,1,0.15],[0,0,1,0.25]],[1000],0,0,"","",_obj];
	blizzard_parent setParticleCircle [0,[0,0,0]];
	blizzard_parent setParticleRandom [0,[40,0,8],[0,0,0],0,0.5,[0,0,0,0.03],0,0];
	// blizzard_parent setDropInterval 0.001;
	blizzard_front = "#particleSource" createVehicleLocal _pos;
	blizzard_front setParticleParams [["a3\data_f\ParticleEffects\Universal\Universal.p3d",16,13,10,0],"","Billboard",1,10,[0,30,8],[0,0,0],0,1.69,1,2,[5],[[1,1,1,.19]],[1000],.7,.3,"","",_obj];
	blizzard_front setParticleCircle [0,[0,0,0]];
	blizzard_front setParticleRandom [0,[40,0,8],[0,0,0],0,0.35,[0,0,0,.05],0,0];
	// blizzard_front setDropInterval 0.005;
	arBlizzard_Emitters = [blizzard_parent,blizzard_front];
	arBlizzard_Drop_Max = [0.001,0.005];
	...
	*/
	...
	//~ blizzard_front setDropInterval 0.001;
	blizzard_parent = "#particleSource" createVehicleLocal _pos;
	...
	blizzard_parent setParticleCircle [0,[0,0,0]];
	blizzard_parent setParticleRandom [0, [25,0,8], [0, 0, 0], 0, .5, [0,0,0,0.03], 0, 0]; // try 25 rather than 15 l/r
	//~ blizzard_parent setDropInterval 0.001;
	...
	blizzard_rear setParticleRandom [0, [15,12, 6], [0, 0, 0], 0, .5, [0,0,0,0.03], 0, 0];// try 12 front/back with -10 above (was 0)
	//~ blizzard_rear setDropInterval 0.002;
	...
	blizzard_right setParticleRandom [0, [20,15, 4], [0, 0, 0], 0, .5, [0,0,0,0.03], 0, 0];// try 20x15 on left and right
	//~ blizzard_right setDropInterval 0.003;
...
MKY_fnc_whiteout_Init = {
	...
	whiteout_parent setParticleParams [["a3\data_f\ParticleEffects\Universal\Universal.p3d", 16,13,6,0],"","Billboard",1,10,[0,30,8],[0,0,0],(0),1.69,1,1.5,[3],[[0,0,1,0.15],[0,0,1,0.25]],[1000],0,0,"","",_obj];
	whiteout_parent setParticleCircle [0,[0,0,0]];
	whiteout_parent setParticleRandom [0,[40,0,8],[0,0,0],0,0.5,[0,0,0,0.03],0,0];
	// whiteout_parent setDropInterval 0.001;
	whiteout_front = "#particleSource" createVehicleLocal _pos;
	whiteout_front setParticleParams [["a3\data_f\ParticleEffects\Universal\Universal.p3d",16,13,10,0],"","Billboard",1,10,[0,30,8],[0,0,0],0,1.69,1,2,[5],[[1,1,1,.19]],[1000],.7,.3,"","",_obj];
	whiteout_front setParticleCircle [0,[0,0,0]];
	whiteout_front setParticleRandom [0,[40,0,8],[0,0,0],0,0.35,[0,0,0,.05],0,0];
	// whiteout_front setDropInterval 0.005;
	arwhiteout_Emitters = [whiteout_parent,whiteout_front];
	arwhiteout_Drop_Max = [0.001,0.005];
	...
	*/
	...
	//~ whiteout_front setDropInterval 0.001;
	whiteout_parent = "#particleSource" createVehicleLocal _pos;
	...
	whiteout_parent setParticleCircle [0,[0,0,0]];
	whiteout_parent setParticleRandom [0, [25,0,8], [0, 0, 0], 0, .5, [0,0,0,0.03], 0, 0]; // try 25 rather than 15 l/r
	//~ whiteout_parent setDropInterval 0.001;
	...
	whiteout_rear setParticleRandom [0, [15,12, 6], [0, 0, 0], 0, .5, [0,0,0,0.03], 0, 0];// try 12 front/back with -10 above (was 0)
	//~ whiteout_rear setDropInterval 0.002;
	...
	whiteout_right setParticleRandom [0, [20,15, 4], [0, 0, 0], 0, .5, [0,0,0,0.03], 0, 0];// try 20x15 on left and right
	//~ whiteout_right setDropInterval 0.003;
