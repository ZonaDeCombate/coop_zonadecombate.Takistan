CHHQ_fnc_deploy = {
	...
	if (count _flatPos isEqualTo 0) exitWith {systemChat format ["Não é possível implantar HQ aqui! Encontre uma posição plana sem quaisquer objetos nas proximidades (%1m).", round _radius]};
	...
	[["Montando HQ"],"BIS_fnc_dynamicText",_nearestPlayers] call BIS_fnc_MP;
	sleep 3;	
	...
	*/
	...
	[[_veh, ["Desmontar HQ", "_this spawn CHHQ_fnc_undeploy", [_side, _cargoInfo, _composition], 0, false, true, "", "[_target, _this] call CHHQ_fnc_actionConditions"]], "CHHQ_fnc_addAction", _side] call BIS_fnc_MP;
	[["RespawnAdded",["DEPLOYMENT POINT",format ["HQ montado no grid %1", mapGridPosition (getPos _veh)],"\A3\ui_f\data\map\markers\nato\b_hq.paa"]],"BIS_fnc_showNotification",_side] call BIS_fnc_MP;
...
CHHQ_fnc_undeploy = {
	...
	[["CHHQ_deployBlackout"],"BIS_fnc_blackOut",_nearestPlayers,false,true] call BIS_fnc_MP;
	[["Desmontando HQ"],"BIS_fnc_dynamicText",_nearestPlayers] call BIS_fnc_MP;
	sleep 3;
	{_x allowDamage false; _x enableSimulationGlobal false} forEach _nearestPlayers;	
...
CHHQ_fnc_startingSetup = {
	...
	if (_veh getVariable ["CHHQ_deployed",false]) then {
		...
		if (playerSide isEqualTo _side) then {		
			...
			_id = _veh addAction ["Desmontar HQ", "_this spawn CHHQ_fnc_undeploy", [_side, _cargoInfo, _composition], 0, false, true, "", "[_target, _this] call CHHQ_fnc_actionConditions"];
			_veh setVariable ["CHHQ_actionID",_id];
		};
	} else {
		_veh lock false;
		...
		if (playerSide isEqualTo _side) then {
			...
			_id = _veh addAction ["Montar HQ", "_this spawn CHHQ_fnc_deploy", [_side, _cargoInfo, _composition], 0, false, true, "", "[_target, _this] call CHHQ_fnc_actionConditions"];	
			_veh setVariable ["CHHQ_actionID",_id];	
...
CHHQ_fnc_updateTeleportActions = {
	...
	_actionIDarray = [];
	{
		...
		_actionText = if (count CHHQ_HQarray > 1) then {format ["Mover para o HQ-%1", _x getVariable ["CHHQ_index", -1]]} else {"Move to HQ"};
...
switch (toLower typeOf _obj) do {
	...
	case (toLower "B_Slingload_01_Medevac_F"): {
